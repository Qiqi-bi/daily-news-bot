...
DEFAULT_INDUSTRY_RADAR: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "why": "AI应用从模型走向硬件执行层时，机器人链可能受益，但必须看订单、BOM成本、良率和量产节奏。",
        "watch": "大客户订单、伺服/减速器/传感器供给、客户验证、交付周期、良率、毛利率和估值拥挤度。",
        "verify": "先看到客户验证、订单或交付放量，再判断是否能从题材变成收入。",
        "action": "放入观察池；不因为AI故事自动扩大进攻仓。",
        "instruments": [],
    },
    {
        "id": "defense_low_altitude",
        "name": "军工/低空/无人机",
        "layer": "watch",
        "theme_keys": ["china_macro"],
        "keywords": [
        ...
        "why": "军工和低空更像政策、预算和订单驱动，不适合只看概念热度。",
        "watch": "正式预算、军品定型、低空试点、无人机订单、航空发动机进度、交付周期和回款。",
        "verify": "至少确认政策/预算/订单/交付中的两项，传闻和展会消息只做观察。",
        "action": "只看可验证订单和预算，不追未确认传闻。",
        "instruments": [],
    },
    {
        "id": "chemicals_fertilizer",
        "name": "化工/化肥/农药",
        "layer": "watch",
        "theme_keys": ["energy", "china_macro"],
        "keywords": [
            "chemical",
            "fertilizer",
            "urea",
        ...
        "why": "化工链受能源、出口、反内卷和农业保供共同影响，重点是价差和利润，不是单条涨价新闻。",
        "watch": "原料价格、产品价格、内外价差、出口限制、库存、开工率、毛利率和政策保供约束。",
        "verify": "先看价格、库存、出口和毛利率是否一起确认，再判断是否进入候选。",
        "action": "涨价新闻先归入观察；没有利润兑现前不生成买卖。",
        "instruments": [],
    },
    {
        "id": "coal_power_security",
        "name": "煤炭/火电/电力安全",
        "layer": "watch",
        "theme_keys": ["energy"],
        "keywords": [
        ...
        "why": "电力安全和调峰约束下，煤炭火电更偏保供、分红和防守，不和新能源成长逻辑混用。",
        "watch": "煤价、长协价格、火电利用小时、容量电价、库存、用电负荷和迎峰度夏/迎峰度冬压力。",
        "verify": "先确认用电负荷、煤价、容量电价和火电利用小时是否同步改善。",
        "action": "作为防守/现金流观察，不把短期煤价波动直接转成交易。",
        "instruments": [],
    },
    {
...
INDUSTRY_FACT_DETAILS: dict[str, dict[str, Any]] = {
    ...
    "embodied_robotics": {
        "逻辑": "机器人和具身智能要从AI叙事落到硬件执行层，核心是客户验证、订单、交付和成本下降。",
        "确认": ["客户验证通过", "伺服/减速器/执行器订单放量", "良率提升", "BOM成本下降"],
        "否定": ["只有展会或概念新闻", "量产延期", "价格竞争压缩毛利", "估值已提前透支"],
    },
    "defense_low_altitude": {
        "逻辑": "军工、低空和无人机看政策预算、定型、订单和交付，不把概念热度当成收入。",
        "确认": ["正式预算或政策落地", "军品定型", "无人机/低空订单确认", "交付和回款改善"],
        "否定": ["只停留在试点或展会", "订单没有公开确认", "回款慢", "估值先于业绩大幅扩张"],
    },
    "chemicals_fertilizer": {
        "逻辑": "化工、化肥和农药看能源成本、出口规则、库存和内外价差，涨价要能转化成毛利。",
        "确认": ["产品价格连续上涨", "库存下降", "出口或内外价差改善", "龙头毛利率修复"],
        "否定": ["政策只保供不保利润", "原料成本吞掉涨价", "出口受限但库存高", "开工率继续上升导致供给释放"],
    },
    "coal_power_security": {
        "逻辑": "煤炭和火电偏电力安全、调峰、分红和防守逻辑，重点看用电负荷、容量电价和现金流。",
        "确认": ["用电负荷上行", "容量电价改善", "火电利用小时提升", "煤价和长协稳定"],
        "否定": ["煤价单日波动", "保供压制利润", "火电利用小时下降", "分红能力恶化"],
    },
