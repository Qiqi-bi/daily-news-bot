...
[![RSS](https://img.shields.io/badge/RSS-Feed-orange.svg)](https://en.wikipedia.org/wiki/RSS)

🤖 **智能新闻采集与分析机器人** | 📈 **AI驱动的新闻洞察** | 🚀 **自动化推送系统**
...
- **多源采集**：支持RSS、API、Playwright浏览器抓取等多种采集方式
- **智能分析**：使用DeepSeek等大语言模型对新闻进行深度分析
- **多层策略**：API优先、RSS源、Playwright抓取、传统RSS四层采集策略
- **智能去重**：自动过滤重复新闻
- **重要性排序**：根据新闻来源、关键词等因素计算重要性分数
- **价格注入**：自动获取相关资产的实时价格
- **飞书推送**：将分析结果以富文本卡片形式推送到飞书群
